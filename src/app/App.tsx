...
import { createRoom as createRoomOnServer, deleteRoom as deleteRoomOnServer, updateRoom as updateRoomOnServer, sendMessage as sendMessageOnServer, subscribeRooms, subscribeMessages, isFirebaseConfigured } from "../lib/firebase";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeRooms((data) => {
      ...
      saveRooms(roomsData);
      if (currentRoom) {
        const nextRoom = roomsData.find((r) => r.id === currentRoom.id);
        if (nextRoom) setCurrentRoom(nextRoom);
      }
    });
    return unsubscribe;
  }, [firebaseEnabled, currentRoom?.id]);

  useEffect(() => {
    if (!firebaseEnabled || !currentRoom) return;
    setMessages(loadMsgs(currentRoom.id));
    const unsubscribe = subscribeMessages(currentRoom.id, (data) => {
      const nextMsgs = data as ChatMessage[];
      setMessages(nextMsgs);
      saveMsgs(currentRoom.id, nextMsgs);
    });
    return unsubscribe;
  }, [firebaseEnabled, currentRoom?.id]);
  ...
  const handleCreateRoom = (name: string, pw?: string) => {
    ...
    if (firebaseEnabled) {
      createRoomOnServer(room).then(finalize).catch(() => setInfoMsg("채팅방 생성에 실패했습니다."));
    } else {
      finalize();
    }
  };
  ...
  const handleDeleteRoom = (r: ChatRoom) => {
    const applyDelete = () => {
      ...
      setRooms(updated);
      saveRooms(updated);
      localStorage.removeItem(K.MSGS(r.id));
      localStorage.removeItem(K.ROOM_PRESENCE(r.id));
      if (currentRoom?.id === r.id) { setCurrentRoom(null); pushNav("chatlist"); }
      if (subRoom?.id === r.id) setSubRoom(null);
      setPendingDeleteRoom(null);
    };
    if (firebaseEnabled) {
      deleteRoomOnServer(r.id).then(applyDelete).catch(() => setInfoMsg("채팅방 삭제에 실패했습니다."));
    } else {
      applyDelete();
    }
  };
  const handleRenameRoom = (r: ChatRoom, name: string) => {
    ...
    if (firebaseEnabled) {
      updateRoomOnServer({ ...r, name }).catch(() => setInfoMsg("채팅방 이름 변경에 실패했습니다."));
    }
  };
  ...
  const handleSendMessage = (text: string, images: string[], videos: string[], audios: string[] = []) => {
    ...
    if (firebaseEnabled) {
      sendMessageOnServer(currentRoom.id, { username: user.username, emoji: user.emoji, text, images, videos, audios, date: msg.date, time: msg.time }).catch(() => setInfoMsg("메시지 전송에 실패했습니다."));
      persist();
    } else {
      persist();
    }
